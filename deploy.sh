...
echo
echo "==> [N-12] Agent 任务快照目录自检..."
# ── N-12 修复（2026-09-18）：状态目录存在性 + 可写性 ────────────────────────
# `.jimeng-agent-tasks` 由 Dockerfile 的 RUN mkdir 创建（属主 jimeng）。
# 但若**镜像未重建**（例如手工只跑 `up -d`），旧镜像里没有该目录，
# 而 /app 自身是 root:root → uid=1001 无法 mkdir → 任务快照落盘失败（warning 级），
# 后果是 `GET /v1/agent/tasks` **跨重启丢失任务列表**。
# 这里做幂等兜底：缺失则以 root 补建并 chown；随后用与服务同一 uid 做真写真删探针。
CDIR=/app/.jimeng-agent-tasks
if docker exec jimeng-api-agent sh -c "test -d $CDIR" 2>/dev/null; then
  echo "✓ [N-12] 状态目录已存在：$CDIR"
else
  echo "==> [N-12] 状态目录缺失（镜像可能未重建）→ 容器内补建并 chown 1001:1001 ..."
  docker exec -u 0 jimeng-api-agent sh -c "mkdir -p $CDIR && chown 1001:1001 $CDIR" \
    && echo "✓ [N-12] 已补建" \
    || echo "⚠️  [N-12] 补建失败 —— 任务快照会失败，可手工执行：docker exec -u 0 jimeng-api-agent sh -c 'mkdir -p $CDIR && chown 1001:1001 $CDIR'"
fi
PROBE=$(docker exec -u 1001 jimeng-api-agent sh -c "touch $CDIR/.probe && rm $CDIR/.probe && echo OK" 2>/dev/null | tr -d '\r')
[ "$PROBE" = "OK" ] \
  && echo "✓ [N-12] uid=1001 真写真删探针通过（任务快照可落盘）" \
  || echo "⚠️  [N-12] uid=1001 对 $CDIR 不可写 —— 任务列表会跨重启丢失"
