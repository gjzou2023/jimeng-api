...
⚠️ 2026-09-18 更正：每个场景的产出张数 = **请求的 `n`**（1~8，默认 1），
   由报文 `abilities.gen_option.gen_count` 控制（旧版缺该字段，上游才兜底出 4 张）。
   服务端响应带 `urls: string[]` 全量字段；多张时命名 `NN_标题_01.png …`。
   可用环境变量 `JIMENG_AGENT_KEEP`（默认 0 = 全存）限制每场景**落盘**张数。
...
def _download_items(items, out: Path, kind: str, strip_wm: bool) -> int:
    ...
    服务端响应带 `urls: string[]` 全量字段（每个场景的产出张数 = 请求的 `n`）。
    旧实现只取 `url`（首图）→ 若上游多给，多出的图**已生成、已计费**却被丢弃（G-2）。
...
def main() -> int:
    ...
    ap.add_argument("--n", type=int, default=None,
                    help="每个场景的**输出张数**（1~8，默认 1）——写进报文 "
                         "abilities.gen_option.gen_count。与 --max-items（场景数）是两个维度")
    # ── 直出单次 ──
