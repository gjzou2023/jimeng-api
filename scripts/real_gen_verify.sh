...
#   L3 张数符合预期 ：上游 items 数 **== 请求的 n**，且 服务端报告落盘数 == items 数
#                     （若 out_dir 本地可见，再与磁盘实际 .png 数交叉核对）
#
# 与旧断言的区别（2026-09-18 二次修正）：
#   最初把"张数 == 环境变量"绑死 → 上游当时固定出 4 张 → 恒红；
#   随后放宽成"≥EXPECT_MIN"→ 会**掩盖修复失效**（出 4 张也算过）。
#   真因查明并修复后（报文缺 `abilities.gen_option.gen_count`），判据回到**严格等式**：
#   L3 = 「返回张数 == 请求的 n」。这既是修复验收，也是上游行为变更的哨兵。
#
# 用法：
...
#   MODEL      模型（默认 jimeng-4.0 —— **1 分/张，最省**；5.0-lite/nanobanana 为 3 分/张）
#   N          请求张数（默认 1；1~8。断言"实际返回张数 == N"）
#
# 成本：1 个场景 × N 张 × 单价。默认 N=1 + jimeng-4.0 → ≈ 1 积分。
# 护栏：
...
case "$N" in
  ''|*[!0-9]*) echo "N 必须是整数（1~8），收到：$N"; exit 2 ;;
esac
if [ "$N" -lt 1 ] || [ "$N" -gt 8 ]; then
  echo "N 必须在 1~8 之间（官网单图可选范围），收到：$N"; exit 2
fi
...
echo "  base     : $BASE"
echo "  model    : $MODEL"
echo "  n        : $N（断言：实际返回张数 == $N）"
echo "  out_dir  : ${OUT_DIR:-（未指定 → 只校验 URL 层）}"
echo

# ---------- 1. 构造请求（nonce 化，绕开配额护栏） ----------
"$PY" - "$NONCE" "$OUT_DIR" "$MODEL" "$N" > "$REQ" <<'PYEOF'
import json, sys
nonce, out_dir, model, n = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4])
...
body = {
    ...
    "max_items": 1,
    "consistency": False,
    # ★ 每个场景的输出张数（1~8）—— 写进报文 abilities.gen_option.gen_count
    "n": n,
}
...
if [ "$HTTP_CODE" != "000" ]; then
  VERDICT="$("$PY" - "$RESP" "$OUT_DIR" "$N" <<'PYEOF'
import glob, json, os, sys

resp_path, out_dir, expected_n = sys.argv[1], sys.argv[2], int(sys.argv[3])
out = []
...
# ---- L3 张数符合预期（严格等式：上游 items == 请求的 n，且落盘数一致）----
ok3 = (n_urls == expected_n) and (n_files == n_urls)
detail = "上游 items=%d，服务端报告落盘=%d，请求 n=%d" % (n_urls, n_files, expected_n)
if n_urls != expected_n:
    detail += " ← ✗ 上游未按 n 出图（检查报文 abilities.gen_option.gen_count，C-1 回归？）"
if n_files != n_urls:
    detail += " ← ✗ 落盘数与产出数不一致"
