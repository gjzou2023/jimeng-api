 ...
 *
 * ⚠️ 2026-09-18 变更（C-7）：**未知模型不再静默降级**。
 * 原实现末行是 `modelMap[model] ? model : defaultModel` —— 国内站传一个拼错的模型名，
 * 会被**悄悄换成默认模型**照常出图：调用方以为用的是 A 模型，实际花的是 B 模型的钱与画质。
 * 现改为明确报错并列出本区域可选模型。
 * 保留的两种"合法回退"（均为**显式可见**、会打日志）：
 *   ① 完全不传 model（或传通用占位符 `"jimeng"`）→ 用区域默认模型（/v1/models 首条就是这个语义）；
 *   ② 国际站收到**国内站默认模型**名 → 回退到国际站默认模型（原有兼容行为，未改）。
 */
export function getModel(model: string, regionInfo: RegionInfo): ModelResult {
  ...
  // C-7 追加：通用占位符 `"jimeng"`（/v1/models 首条）语义为"用区域默认模型"，同样走这条分支。
  if (!model || model === "jimeng") {
    logger.info(`未指定具体模型（model=${JSON.stringify(model)}），使用区域默认模型 "${defaultModel}"`);
  ...
  // ★ C-7：未知模型**明确报错**（原实现此处为 `modelMap[model] ? model : defaultModel`，会静默降级）
  if (!modelMap[model]) {
    const supportedModels = Object.keys(modelMap).join(', ');
    throw new Error(
      `不支持的模型 "${model}"。本区域支持的模型: ${supportedModels}。`
      + `如需使用区域默认模型（${defaultModel}），请显式传 model:"jimeng" 或省略 model 参数。`
    );
  }
 ...
 * 图生图
 *
 * n（可选，1~8，默认 1）：**输出张数**，写入 `abilities.gen_option.gen_count`。
 * ⚠️ 2026-09-18 行为变更（C-1）：本路径此前**不发送** gen_count → 上游按模型兜底出 4 张。
 * 现在默认 1 张，与官网图片模式（含参考图）默认值一致；如需多张请显式传 n。
 */
export async function generateImageComposition(
  ...
  images: (string | Buffer)[],
  {
    ...
    /** 输出张数（1~8）。未传 → 环境变量 JIMENG_BENEFIT_COUNT → 默认 1 */
    n?: number;
  },
  refreshToken: string
) {
  ...
  const plannedImageCount = resolveRequestImageCount(n);
  logger.info(`使用模型: ${userModel} 映射模型: ${model} 图生图功能 ${imageCount}张输入图 → 输出 ${plannedImageCount} 张 ${resolutionResult.width}x${resolutionResult.height} 精细度: ${sampleStrength}`);
  ...
  // 轮询结果
  // ★ 与报文 `gen_count` 同源（plannedImageCount）。
  // SmartPoller 的 expectedItemCount 只是"提前收工"的加速阈值，**不是闸门**：
  // 状态为 10/50 时立即退出（smart-poller.ts:116），因此即使上游给少于期望张数也不会挂住。
  const poller = new SmartPoller({
    maxPollCount: 900,
    pollInterval: 10000, // 10秒轮询间隔
    expectedItemCount: plannedImageCount,
    type: 'image',
 ...
 *
 * n（可选，1~8，默认 1）：**输出张数**。写入报文 `component_list[0].abilities.gen_option.gen_count`
 * （2026-09-18 C-1 修复：此前该字段**从未发送**，上游因此取模型兜底值 4，表现为"恒出 4 张、扣 4 份"）。
 *
...
export async function generateImages(
  ...
  return await generateImagesInternal(userModel, prompt, { ratio, resolution, sampleStrength, negativePrompt, intelligentRatio, mode, quotaMode, n }, refreshToken, meta);
}
...
async function generateImagesInternal(
  ...
  if (imageMode === "single") {
    ...
    if (groupEligibleByKeyword) {
      ...
      // ⚠️ 2026-09-18 文案更正（C-8）：旧文案称"单图模式上游固有产出仍为 4 张、无法减少"，
      // 该结论已被证伪（真因是报文缺 `gen_count`，现已补齐）。此处改为陈述当前真实语义。
      const hint =
        `本次请求未显式声明生成模式（mode），但提示词含组图特征：${reasons.join("；")}。`
        + `本次已按【单图】执行：输出张数由参数 n 决定（默认 1，上限 ${MAX_SINGLE_IMAGE_COUNT} 张）。`
  ...
  //
  // ★ 张数唯一解析点（C-1）：显式入参 n ＞ 环境变量 JIMENG_BENEFIT_COUNT ＞ 默认 1。
  // 下面的「配额预估 / 埋点 benefitCount / 报文 gen_count / 轮询 expectedItemCount」
  // **全部取自这一个变量**，杜绝三处各读一遍导致的「生成 N 张却等 M 张」挂起。
  const plannedImageCount = resolveRequestImageCount(n);
  logger.info(
    `[image-count] 本次输出张数 n = ${plannedImageCount}`
    + `（来源：${n !== undefined ? "请求参数 n" : "环境变量 JIMENG_BENEFIT_COUNT/默认 1"}；`
    + `上限 ${MAX_SINGLE_IMAGE_COUNT}）`
  );
  ...
  // 使用 payload-builder 构建 draft_content
  // ★ genCount 即张数开关：写入 component_list[0].abilities.gen_option.gen_count
  const draftContent = buildDraftContent({
    componentId,
  ...
  const poller = new SmartPoller({
    maxPollCount: 900,
    pollInterval: 10000, // 10秒轮询间隔
    // ★ 与报文 `gen_count` 同源：都取自上面的 plannedImageCount。
    // ⚠️ 2026-09-18 更正（C-8，推翻本处旧注释）：
    //   旧注释称"该开关不控制上游实际产出张数，三模型固定返 4 张"——**已证伪**。
    //   真因是报文里**缺** `abilities.gen_option.gen_count`（我们从未发送过），
    //   上游遂取模型兜底 `default_generate_count = 4`。补上该字段后 n 即生效
    //   （参考实现实测：n=1 → 出 1 张 / 扣 1 份；修前是 4 张 / 4 份）。
    //   若线上回归发现 n 未生效，本处与 gen_count 会同时失准，属"同源失效"，便于定位。
    expectedItemCount: plannedImageCount,
    type: 'image',
    timeoutSeconds: 1800 // 30 分钟超时
  });
...
async function generateJimeng4xMultiImages(
  ...
  if (countCheck.state !== "ok") {
    ...
    throw new APIException(
      ...
      // ⚠️ 2026-09-18 文案更正（C-8）：旧文案称"上游单请求固有产出 4 张、换模型也不会变少"，
      // 该结论源自"报文缺 gen_count"的误判，已证伪。现改为给出正确、可执行的做法。
      + `若只想生成 ${MAX_SINGLE_IMAGE_COUNT} 张以内：请去掉提示词中的组图关键词，并显式传 mode:"single"，`
      + `用参数 n 指定张数（默认 1，上限 ${MAX_SINGLE_IMAGE_COUNT}）。`
      + "注意：模型只影响画质与单价，**不决定张数**——想少出图应调 n，而不是换模型。"
    );
  }
  ...
  // 使用 payload-builder 构建 draft_content
  // C-1 顺带对齐：组图同样写入 `abilities.gen_option.gen_count`，取值 = 提示词解析出的张数。
  // 两者语义一致（gen_count 就是"要几张"），不会互相冲突；TBD：本路径**尚无真机回归证据**，
  // 首次上线后需单独验证「组图 n=3 是否真出 3 张」。
  const draftContent = buildDraftContent({
    componentId,
    generateType: "generate",
    coreParam,
    genCount: targetImageCount,
  });
