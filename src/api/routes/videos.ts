...
import { assertPromptQuota, commitPromptQuota } from '@/api/guards/prompt-quota.ts';
...
export default {
    ...
    post: {
        ...
        '/generations': async (request: Request) => {
            ...
            // 同一提示词产出配额护栏（单视频路径：默认 1 段、同提示词累计 ≤ JIMENG_PROMPT_QUOTA_MAX，默认 4）
            // 目的：防止「生成超时 → 用户重试 → 同一提示词被反复提交」造成积分重复消耗。
            // 上游一次调用只产 1 段（videos.ts 内 expectedItemCount: 1），故本次产出恒为 1。
            assertPromptQuota(prompt, "single", "video", 1);
            ...
            // 生成成功才记账（失败/抛错不占用额度）
            commitPromptQuota(prompt, "single", "video", 1);
