...
import { generateImages, generateImageComposition, ImageGenMeta } from "@/api/controllers/images.ts";
import { DEFAULT_IMAGE_MODEL } from "@/api/consts/common.ts";
import { resolveRequestImageCount } from "@/api/builders/payload-builder.ts";
  ...
  post: {
    "/generations": async (request: Request) => {
      ...
      request
        ...
        // n: 输出张数（1~8）。这里**只挡"完全不是标量"的类型**（对象/数组/布尔），
        // 其余（含 "abc"、0、9、1.5）一律放行给 resolveRequestImageCount 生成统一直观的报错。
        // 2026-09-18 修正：原先用 _.isFinite 会把 "abc" 挡成通用的 “Params body.n invalid”，
        // 与数值越界的 “必须是 1-8 之间的整数” 口径不一致 —— 同一个参数两类提示，调用方困惑。
        .validate("body.n", v => _.isUndefined(v) || typeof v === "string" || typeof v === "number")
        // mode: "single" 强制单图 / "group" 强制组图 / "auto" 或缺省 = 兼容模式（按关键词推断）
        .validate("body.mode", v => _.isUndefined(v) || (_.isString(v) && ["single", "group", "auto"].includes(v)))
      ...
      // ★ n 的唯一解析点（越界/非整数在此明确报错，不静默截断）
      const requestedCount = resolveRequestImageCount(n);

      const responseFormat = _.defaultTo(response_format, "url");
      // meta 为出参：收回"本次实际走的路径 + 可读提示"，用于回给调用方（不再静默切换）
      ...
      return {
        ...
        data,
        mode: meta.mode,
        // 出参回显本次请求张数，便于调用方对账（与报文 gen_count 同源）
        n: requestedCount,
        // 仅在"提示词含组图特征但未显式声明 mode"时出现，不影响既有调用方
    ...
    "/compositions": async (request: Request) => {
      ...
      if (isMultiPart) {
        request
          ...
          .validate("body.response_format", v => _.isUndefined(v) || _.isString(v))
          // multipart 下字段是字符串，故同时接受 "3"
          // multipart 下字段是字符串；同样只挡非标量，语义交给 resolveRequestImageCount（统一可读报错）
          .validate("body.n", v => _.isUndefined(v) || typeof v === "string" || typeof v === "number")
          .validate("headers.authorization", _.isString);
      } else {
        request
