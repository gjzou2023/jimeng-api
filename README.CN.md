...
**请求参数**:
- `model` (string, 可选): 使用的模型名称。**国内站默认 `jimeng-5.0-lite`**（对应官网「图片 5.0 Lite」）；国际站（US/HK/JP/SG）默认 `jimeng-4.5`。可用清单见 `GET /v1/models`。传入未知模型将**明确报错**（不再静默降级）。
...
- `intelligent_ratio` (boolean, 可选): 是否启用智能比例，默认为 `false`。**⚠️ 此参数仅对 jimeng-4.0/jimeng-4.1/jimeng-4.5/jimeng-4.6/jimeng-5.0/jimeng-5.0-lite 模型有效，其他模型将忽略此参数。** 启用后系统会根据提示词自动推断最佳图像比例（例如："竖屏" → 9:16，"横屏" → 16:9）。
- `negative_prompt` (string, 可选): 负面提示词
- `sample_strength` (number, 可选): 采样强度 (0.0-1.0)
- `response_format` (string, 可选): 响应格式 ("url" 或 "b64_json")
- `n` (number, 可选): **生成张数，默认 `1`，范围 `1–8`**（与官网「图片 5.0 Lite」单图可选张数一致）。
  ⚠️ 越界或非整数会**明确报错**（不静默截断）。该值最终写入报文的
  `abilities.gen_option.gen_count`——**注意必须在该层级**，放在 `component` 级或
  `abilities.generate` 内都会被上游忽略，从而回落到模型默认产出（4.x/5.x = 4 张）。
  响应体的 `n` 字段会回显实际生效值。
- `mode` (string, 可选): `"single"`（单图，默认）/ `"group"`（组图，上限 15，张数写在提示词里）。
...
- `nanobanana`: 仅国际站支持
- `jimeng-5.0-lite`: **国内站默认模型**（内部键 `high_aes_general_v50`，对应官网「图片 5.0 Lite」）
- `jimeng-5.0`: 国内站及亚洲国际站 (HK/JP/SG) 支持
- `jimeng-4.6`: 国内站及亚洲国际站 (HK/JP/SG) 支持
- `jimeng-4.5`: 国内、国际站均支持，支持 2k/4k 全部 ratio 及 intelligent_ratio **（国际站默认模型）**
...
- `resolution` (string, 可选): 分辨率级别，默认为 `"2k"`。支持的分辨率: `1k`, `2k`, `4k`。
- `intelligent_ratio` (boolean, 可选): 是否启用智能比例，默认为 `false`。**⚠️ 此参数仅对 jimeng-4.0/jimeng-4.1/jimeng-4.5/jimeng-4.6/jimeng-5.0/jimeng-5.0-lite 模型有效，其他模型将忽略此参数。** 启用后系统会根据提示词和输入图片自动调整输出比例。
- `negative_prompt` (string, 可选): 负面提示词
- `sample_strength` (number, 可选): 采样强度 (0.0-1.0)
