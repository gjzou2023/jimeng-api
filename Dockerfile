...
# 创建应用需要的目录并设置权限
# ⚠️ N-12（2026-09-18）：`/app/.jimeng-agent-tasks` 是 **Agent 任务快照目录**，与 `/app/output`（产物目录）
# **职责不同**：前者存任务状态 JSON（供 GET /v1/agent/tasks 跨重启恢复），后者存生成出来的图片。
# 二者都必须由 uid=1001 可写，否则：
#   - output 不可写 → 落盘静默失败（N-11，已修）
#   - 快照目录不可写 → 任务列表跨重启丢失（N-12，本条修复）
# 注意：`/app` 自身是 root:root，非 root 用户**无法**在其中 mkdir 子目录 —— 必须在此显式创建。
RUN mkdir -p /app/logs /app/tmp /app/output /app/.jimeng-agent-tasks && \
    chown -R jimeng:nodejs /app/logs /app/tmp /app/output /app/.jimeng-agent-tasks
...
# ⚠️ N-12（2026-09-18）：显式固定 Agent 任务快照目录。不设时 `tasks.ts` 会回落到
# `path.join(process.cwd(), ".jimeng-agent-tasks")` = `/app/.jimeng-agent-tasks`；
# 虽同值，但**显式声明**可避免后续 WORKDIR/启动方式变更时静默漂移到不可写位置。
# 本变量是 N-12 的**双保险**：即便镜像未被重建（仅 `up -d`），compose 侧设置的同名变量亦会生效。
ENV JIMENG_AGENT_TASK_DIR=/app/.jimeng-agent-tasks
...
#
# ⚠️ N-12（2026-09-18）：`/app/.jimeng-agent-tasks`（状态目录）**不做 bind mount**，
# 故不受上述宿主属主问题影响 —— 它由上面的 `RUN mkdir` 直接建好并 chown。
