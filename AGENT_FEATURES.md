...
|---|------|-------------|--------|--------|------------------------|
| **M1** | 单图 | 1 次 | **1** | **4** | `POST /v1/images/generations`（默认） |
...
一句话理解：**M1/M2 = 出 1 个（可到 4）**；**M3 = 一次调用出多张关联图（≤15）**；**M4 = 多次调用垒量（≤40）**。
...
2. 编排层只取 `urls[0]`，静默切换会导致**多出的图已生成、已计费、URL 被丢弃**且日志不报错。
...
```jsonc
{
  ...
  "data": [ { "url": "..." } ],
  "mode": "single",
  "hint": "本次请求未显式声明生成模式（mode），但提示词含组图特征：关键词命中「连续」。为让产出张数可预期，本次已按【单图】执行（默认 1 张）。若确实需要一次生成多张内容关联图，请在请求体中显式传 mode:\"group\"（上限 15 张）；若需要跨场景堆量（最多 40 张），请改用 POST /v1/agent/tasks。"
}
```
...
| 路径 | 触发条件 | 张数决定方式 |
|------|----------|--------------|
| **单图路径（M1）** | 默认；或显式传 `mode:"single"` | `JIMENG_BENEFIT_COUNT`（默认 `1`），上限 **`MAX_SINGLE_IMAGE_COUNT` = 4** |
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `JIMENG_BENEFIT_COUNT` | `1` | **每请求生成张数开关（单图路径 M1）**。默认 `1`：批量场景每场景只出 1 张，避免 4 倍计费。想恢复「生成 4 张候选、4 选 1 挑选」设为 `4`。上限 **4**（`MAX_SINGLE_IMAGE_COUNT`，超出自动截断；<br>⚠️ **2026-09-17 更正**：原文写"上限 40"，那是三条路径共用常量时代的错误口径，已改为 4）。该开关由 `getImageCountPerRequest()` **单一函数**读取，`buildCoreParam` 的 `benefitCount` 与 poller `expectedItemCount` 都调用它——改一处即全生效，不会出现空等挂起。 |
...
| 11 | **编排隔离断言**（`mode:"single"` + `jimeng-4.5` + 组图特征 → 只出 1 张，不静默丢图） | **是** |
