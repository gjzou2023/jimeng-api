...
if [ -n "$TOKEN" ] && command -v python3 >/dev/null 2>&1; then
  ...
  [ "$EXPECT" -gt 4 ] && EXPECT=4   # 超过单图上限 MAX_SINGLE_IMAGE_COUNT 时按 4 钳制
...
echo "== 10. 单图上限钳制（JIMENG_BENEFIT_COUNT 超上限时必须被钳到 4）=="
if [ -n "$TOKEN" ] && command -v python3 >/dev/null 2>&1; then
  # 以 mode:"single" 显式走单图路径；即便服务端 JIMENG_BENEFIT_COUNT=40，也不得超过 4 张。
...
print(len(items) if isinstance(items, list) else '-1')
" 2>/dev/null)
  if [ "${N10:-0}" -ge 1 ] 2>/dev/null && [ "${N10:-99}" -le 4 ] 2>/dev/null; then
    ok "单图路径只返回 $N10 张（≤ MAX_SINGLE_IMAGE_COUNT=4），上限钳制生效"
  else
    no "单图路径返回 ${N10:-?} 张，超出单图上限 4（常量拆分未生效？）"
  fi
else
  echo "  (跳过：未提供 TOKEN 或缺少 python3)"; fi
