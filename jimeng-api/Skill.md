...
- `--model`: Model to use (default: `jimeng-5.0-lite`, 对应官网「图片 5.0 Lite」；国际站请用 `jimeng-4.5`)
  - Options: `jimeng-5.0-lite`, `jimeng-5.0`, `jimeng-4.6`, `jimeng-4.5`, `jimeng-4.1`, `jimeng-4.0`, `jimeng-3.1`, `jimeng-3.0`, `nanobanana` (international only)
- `--ratio`: Aspect ratio (default: `1:1`)
  - Options: `1:1`, `4:3`, `3:4`, `16:9`, `9:16`, `3:2`, `2:3`, `21:9`
- `--resolution`: Resolution level (default: `2k`)
  - Options: `1k`, `2k`, `4k`
- `--n`: **生成张数 1–8（默认 `1`）★ 成本主开关**。不发送时上游按模型兜底产出 4 张（4.x/5.x），
  也就是**默认会扣 4 份额度**；显式传 `--n 1` 即只扣 1 份。该值最终写入报文
  `abilities.gen_option.gen_count`（层级必须是 `abilities` 下、与 `generate` 平级）。
- `--intelligent-ratio`: Enable smart ratio detection based on prompt keywords **⚠️ Only works for jimeng-4.0/jimeng-4.1/jimeng-4.5/jimeng-4.6/jimeng-5.0/jimeng-5.0-lite models; other models will ignore this parameter**
...
**⚠️ IMPORTANT**: This feature only works with the `jimeng-4.0`, `jimeng-4.1`, `jimeng-4.5`, `jimeng-4.6`, `jimeng-5.0`, and `jimeng-5.0-lite` models. Other models (jimeng-3.0, nanobanana, etc.) will ignore the `--intelligent-ratio` flag.
...
- The `--intelligent-ratio` flag only works with `jimeng-4.0`, `jimeng-4.1`, `jimeng-4.5`, `jimeng-4.6`, `jimeng-5.0`, and `jimeng-5.0-lite` models
- Other models (jimeng-3.0, nanobanana, etc.) will ignore this parameter
- Solution: Use one of the supported models if you need intelligent ratio detection

**"额度被多扣 / 一张提示词扣了 4 份"**
- 根因：请求报文缺 `abilities.gen_option.gen_count`，上游回落到模型的 `default_generate_count`（4.x/5.x **= 4**）
- 处置：CLI 显式传 `--n 1`（1–8）。响应体 `n` 字段会回显实际生效值，可对账
