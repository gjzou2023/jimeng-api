...
def generate_text_to_image(
    ...
    Args:
        ...
        image_count: 生成张数 1-8（默认 1）。不传则由服务端环境变量决定，
            服务端默认 1 —— 显式传 1 是**降本主杠杆**（不传时上游按模型兜底产出 4 张）。
...
def generate_image_to_image(
    ...
    Args:
        ...
        api_url: Jimeng API base URL
        output_dir: Output directory for downloaded images
        image_count: 生成张数 1-8（默认 1）。不传则回落到服务端默认，显式传 1 可
            避免上游按模型兜底产出 4 张（降本主杠杆）。
...
def main():
    ...
    for subparser in [text_parser, image_parser]:
        ...
        subparser.add_argument(
            "--model",
            type=str,
            default="jimeng-5.0-lite",
            choices=["jimeng-5.0-lite", "jimeng-5.0", "jimeng-4.6", "jimeng-4.5", "jimeng-4.1", "jimeng-4.0", "jimeng-3.1", "jimeng-3.0", "nanobanana"],
            help="Model to use (default: jimeng-5.0-lite, 对齐官网「图片 5.0 Lite」; 国际站用 jimeng-4.5)"
        )
        subparser.add_argument(
            "--n",
            type=int,
            ...
            help="生成张数 1-8（默认 1，CLI 恒显式发送）。★ 降本主杠杆：不发送时上游按模型兜底产出 4 张（4.x/5.x）"
        )
        subparser.add_argument(
            "--ratio",
            type=str,
